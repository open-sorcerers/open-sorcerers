...
test.before(() => {
	skeletal.setGenerator('', {});
	skeletal.setGenerator('bad-actions-function', {
		actions: () => {
			'bad actions output';
		}
	});
...
test('generator should not be able to run promps if it has none', co.wrap(function* (t) {
	const generatorOne = skeletal.getGenerator('generator-1');
...
test('generator should not be able to run invalid actions data', co.wrap(function* (t) {
	const generatorBadActions = skeletal.getGenerator('bad-actions-function');
