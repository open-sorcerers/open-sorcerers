...
const runner = (bonefileApi, flags) => {
  ...
  const runGeneratorPrompts = bequeath.wrap(function* genPrompts(
    ...
    if (typeof prompts === "function") {
      return yield prompts(bonefileApi.inquirer)
    }
  ...
  const runGeneratorActions = bequeath.wrap(function* genActions(
    ...
    for (const [actionIdx, action] of actions.entries()) {
      ...
      try {
        const actionResult = yield executeActionLogic(
          actionLogic,
          actionCfg,
          data
        )
  ...
  // handle action logic
  const executeActionLogic = bequeath.wrap(function*(action, cfg, data) {
    const type = cfg.type || ""
    ...
    return yield Promise.resolve(action(data, cfg, bonefileApi))
      .then(
  ...
  // request the list of custom actions from the bonefile
  /**
   * @example
   */
  function getCustomActionTypes() {
    return bonefileApi.getActionTypeList().reduce(function(types, name) {
      types[name] = bonefileApi.getActionType(name)
      return types
    }, {})
  }
