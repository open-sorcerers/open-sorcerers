...
test('verify good bypass input', function (t) {
	const [promptsAfterBypassOne, bypassOne] = promptBypass(prompts, ['0'], skeletal);
	t.is(bypassOne.list, 'a');
	t.is(promptsAfterBypassOne.length, 4);
	t.is(promptsAfterBypassOne[0].type, undefined);

	const [promptsAfterBypassTwo, bypassTwo] = promptBypass(prompts, ['b', 'something'], skeletal);
	t.is(bypassTwo.list, 'B');
	t.is(bypassTwo.input, 'something');
	t.is(promptsAfterBypassTwo.length, 3);

	const [promptsAfterBypassThree, bypassThree] = promptBypass(prompts, ['b', 'something', 'something filtered'], skeletal);
...
test('verify bad bypass input', function (t) {
	...
	t.throws(() => promptBypass(prompts, ['a', 'something', 'something filtered', 'something else'], skeletal));
});
