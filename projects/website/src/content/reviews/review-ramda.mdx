...
You can also get the same functionality by playing with `flip`,but IMO this is slightly harder to read.
...
##### Safe property access with `pathOr` and `propOr` 
...
##### Assert stuff with `propEq` and `propSatisfies` and `ifElse`
