...
# Going From Imperative to Functional JavaScript

We've talked a bit about why you'd want to move to the functional programming (FP) paradigm in a [previous article](), but in short, with FP:

1. Everything is functions all the way down. If they're more than a unary function i.e. `(a, b) => a + b`, they should be [curried](/glossary/curry). The reason that they should be functions has a lot to do with [category theory](//en.wikipedia.org/wiki/Category_Theory) which you should read about if you're interested but is beyond the scope of this article.
2. Ideally those functions are [pure functions](/glossary/pure-function). Pure functions are deterministic, meaning that with the same inputs they return the same outputs, they have no side-effects, meaning they do not make change external to the body of the function and they are [referentially transparent](/glossary/referential-transparency) so if you know the inputs you can compare them transparently.
