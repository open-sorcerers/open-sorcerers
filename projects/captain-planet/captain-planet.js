...
var encaseN2 = curry(function (fn) { return function (xx) { return function (yy) { return F__default.node(function (c) { return fn(xx, yy, c); }); }; }; });
// read a file in the Future
var read = function (x) { return encaseN2(fs.readFile)(x)("utf8"); };
// write a file in the Future
var write = curry(
  function (fd, data) { return new F__default(function (bad, good) {
      fs.writeFile(fd, data, "utf8", function (e, f) { return (e ? bad(e) : good(f)); });
      return function () {}
    }); }
);

var CAPTAIN_PLANET_CONFIG = "captain-planet.config.json";
var CAPTAIN_PLANET_RELATIVE_TO_YOUR_HEART = path$1.resolve(
  __dirname,
  CAPTAIN_PLANET_CONFIG
);

// given per file pollution and all pollution, compare and generate a discrete list
var getPollutionMap = curry(function (pollutionByFiles, knownPolluters) { return map(function (ref) {
    var file = ref[0];
    var pp = ref[1];

    var newPollution = difference(pp, knownPolluters);
    var pollution = difference(pp, newPollution);
    return {
      file: file,
      newPollution: newPollution,
      pollution: pollution
    }
  }, pollutionByFiles); }
);
...
var takePollutionDownToZero = function (getter) { return function (acc, ref) {
  var envVar = ref[0];
  var file = ref[1];

  var clone = merge(acc, {});
  var keys = getter(envVar);
  forEach(function (k) {
    var item = clone[k];
    var manyToMany = isArray(item.env);
...
var appendFile = function (file) { return function (x) { return [x, file]; }; };

// we're doing work even while failures might exist
var segmentByKnownPolluters = function (ref) {
  var file = ref.file;
  var newPollution = ref.newPollution;
  var pollution = ref.pollution;

  var isKnown = newPollution.length > 0;
  var addFileToStuff = map(appendFile(file));
  return isKnown
...
var traceSingularPolluter = curry(function (file, item) {
  var files = item.files; if ( files === void 0 ) files = [];
  return uniq(files.concat(file))
});

// multi polluters are the latter
var traceMultiPolluter = curry(function (envVar, file, item) {
  var obj;

  var files = item.files; if ( files === void 0 ) files = {};
  return merge(files, ( obj = {}, obj[file] = uniq((files[file] || []).concat(envVar)), obj ))
});

// take data and make it [data, [].concat(data.env)]
var structureConfigData = pipe(
  box,
  ap([identity, map(pipe(prop("env"), unless(isArray, box)))])
);

// this is definitely not as efficient a lookup as it could be, as we're doing a separate reduce
// inside a reduce
var foldData = curry(function (raw, actual) {
  var getter = getKeysFromEnvVar(raw);
  return reduce(
    function (ref, ref$1) {
      var bad = ref[0];
      var outcome = ref[1];
      ...
      var mapped = reduce(takePollutionDownToZero(getter), raw, mapping);
...
var dealWithPollution = function (data) { return function (ref) {
    var raw = ref[0];
    var knownPollution = ref[1];
...
var lootingAndPollutingIsNotTheWay = pipe(
  reduce(
    function (ref, ref$1) {
      ...
      var file = ref$1[1];
      return ({
      mapping: merge(mapping, ( obj = {}, obj[file] = (mapping[file] || []).concat(poll), obj )),
...
var printErrors = pipe(
  ...
  map(
    function (ref) {
        ...
        return ("* " + (kleur.red(file)) + " - " + (pollution.map(kleur.yellow).join(", ")));
}
  ),
  join(kleur.white("\n"))
);
...
var getKeysFromEnvVar = curry(function (lookup, envVar) { return pipe(
    toPairs,
    filter(function (ref) {
        var v = ref[1];
    ...
    reduce(function (keys, ref) {
      var k = ref[0];

      return keys.concat(k);
    }, [])
  )(lookup); }
);

var useMagicRings = ifElse(
  // problems?
  pipe(getProblems, length, lt(0)),
  // tell captain planet
  pipe(getProblems, lootingAndPollutingIsNotTheWay, F__default.reject),
...
var captainPlanet = pipe(
  // split ripgrep input by newlines
  split(C.n),
  // cut apart the [filename, process.env.?] match
  map(function (z) { return z && [z.slice(0, z.indexOf(":")), /process\.env\.(\w+)/.exec(z)]; }),
  smooth,
  ...
  reduce(
    function (a, ref) {
      ...
      var k = ref[0];
      var v = ref[1];
      return merge(a, ( obj = {}, obj[k] = a[k] ? uniq(a[k].concat(v).sort()) : [v], obj ));
},
    {}
  )
);

var fork = curry(function (bad, good, eff) { return F.fork(bad)(good)(eff); });
